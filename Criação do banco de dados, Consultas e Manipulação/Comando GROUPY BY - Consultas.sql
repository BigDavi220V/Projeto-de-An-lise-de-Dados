...
select count(*) as num_de_clientes,state as "Estado"
from sales.customers
group by state
order by num_de_clientes desc;

-- (Exemplo 2) Contagem agrupada de várias colunas
-- Calcule o nº de clientes por estSado e status profissional 
select state, professional_status, count(*) as num_de_clientes
from sales.customers
group by state, professional_status
order by state, professional_status desc;
